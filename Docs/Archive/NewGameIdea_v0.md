...
Overview
This document serves as the core reference for developing a lightweight, browser-based top-down adventure game inspired by classic Zelda-style exploration. The game features procedurally generated worlds driven by a local LLM (BitNet b1.58 2B4T) for entropy, ensuring unique, replayable sessions. It’s designed for educational fun, aimed at a child (your daughter), with quizzes integrated into gameplay. The core loop involves exploring tiles, collecting items, solving obstacles, and interacting with NPCs via short chats or challenges.
Key Goals:
	•	Simplicity & Performance: Pure JS/Canvas for rendering, runnable on an 8th Gen Intel i7 with 16GB RAM and NVIDIA GTX 1050 2GB. Browser-native for easy prototyping and sharing.
...
Tech Stack
	•	Rendering: HTML5 Canvas (2D context) for all visuals. No external engines—pure JS.
	•	Assets: Bootstrap Icons/Twemoji for emojis, plus LLM-generated SVGs for custom tiles/sprites. Pre-cache as Image objects.
	...
	•	Dependencies: None external—vanilla JS. Optional: math.js for quiz verification if needed.
Game Mechanics
Core Loop
	...
	2	Exploration: Top-down view, player at center. Move to tile edges triggers new chunk gen async.
...
Educational Quizzes
	...
	•	Backend: Local JS array/JSON library (100-500 Q&A pairs, curated from open sources).
	•	LLM Role: Rephrase Q in fun/story way (prompt: “As wise owl, rhyme this: [original Q]”).
	•	Verification: JS code (e.g., math.js/eval for math; string match for others). No LLM math!
...
Tile Builder Rules
	1	Base Gen: Hash to 2D density map; Perlin noise (JS impl) for smooth distribution.
...
Rendering
	...
	•	Animations: Frame swaps (e.g., water ripple: 3 SVGs cycled).
	•	HUD: Top bar for coins, inventory icons; bottom for chat box.
UI, Menus, Pause, & Save
	•	Menus:
	◦	Start: Title screen (play, load, options). Options: Volume, zoom, difficulty (quiz level).
...
Code Structure
Break into small, tight files (1-2k lines max) for copilot ease. Use ES modules (import/export). Folder: src/ for JS, assets/ for SVGs, styles/ for CSS (minimal, e.g., Canvas full-screen).
	•	index.html: Entry (Canvas element, script imports).
	•	styles/main.css: Canvas styling (full-window, no scroll).
	•	src/main.js: Init (load assets, start loop), game state object.
	•	src/render.js: Draw functions (renderChunk, hud, animations).
	•	src/gen.js: World gen (generateChunk, rules, hashing). Worker-wrapped for async.
	•	src/llm.js: WebLLM setup, prompt functions.
	•	src/mechanics.js: Player movement, collisions, interactions.
	•	src/quiz.js: Library data, verification logic, LLM wrapping.
	•	src/inventory.js: Item handling, templates.
	•	src/ui.js: Menus, pause, save/load.
	•	src/audio.js: Sound effects/mixer.
	•	src/utils.js: Helpers (hash, Perlin, BFS).
Style: Clean functions (e.g., function applyMeshing(grid) { ... }), no globals (pass state), comments per module.
Asset Generation
	...
	◦	Library: 20-30 types; store as data URIs in JS const.
...
This bible captures our discussions—iterate as needed during dev! If building, start with gen.js and render.js for a quick prototype.
