...
Key Considerations:
	...
	•	Fallback: If LLM fails, use pure JS RNG for basic generation to maintain playability.
	•	Testing: Benchmark latency in src/llm.js; add UI loading indicators for init/gen.
...
How It Works
	...
	•	Game JS sends prompts via fetch to http://localhost:8080 and receives text outputs.
	•	Outputs are hashed in src/gen.js for world seeding.
Setup Steps
	...
	◦	If no built-in server, wrap with Python/Flask: from flask import Flask, request
	...
	◦	    app.run(port=8080)
	◦	
	4	Game Integration in src/llm.js:
	◦	Async function for calls: async function generateNonsense(prompt) {
	◦	    try {
	...
	◦	    } catch (error) {
	◦	        console.error(error);
	◦	        return fallbackRNG();  // Pure JS random string as backup
	◦	    }
	◦	}
	◦	
	◦	// Example call in gen.js: const text = await generateNonsense('Generate 1-2 absurd sentences from ascend flux');
	◦	
...
Setup Steps
	...
	◦	Compile: emcc -o bitnet.wasm [source_files.c] -s WASM=1 -s EXPORTED_FUNCTIONS='["_malloc", "_free", "_generate"]' -s EXPORTED_RUNTIME_METHODS='["ccall", "cwrap"]' -O3 -s ALLOW_MEMORY_GROWTH=1
	◦	
	▪	Outputs: bitnet.wasm and bitnet.js (loader).
	◦	For WebGPU: Port ternary kernels to WGSL shaders (advanced; use tools like Tint). Integrate via WebGPU API in JS.
	2	Host Files:
	...
	◦	Custom loader in JS: Fetch GGUF, parse to ArrayBuffers (use libraries like gguf-js if available).
	4	Game Integration in src/llm.js:
	◦	Init in Worker: // llm-worker.js (Web Worker)
	...
	◦	worker.onmessage = (e) => { /* Handle ready/output */ };
	◦	
	◦	async function generateNonsense(prompt) {
