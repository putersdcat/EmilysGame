...
The world of Emily's Game is built from a three-tier spatial hierarchy. Each tier serves a distinct purpose in the generation pipeline, and together they form a **tile grammar** — a formal system of composable, rule-governed pieces that can produce vast, varied, and coherent worlds from a small set of primitives.

The three tiers are:

- **Micro Tiles (L0)** — The atomic visual and logical unit. One grid cell. The smallest thing that has meaning.
- **World Unit Tiles (L1)** — A 5×5 composite of micro tiles. The building block of local structure. The thing that creates recognizable shapes.
- **Macro Tiles (L2)** — A 5×5 composite of world unit tiles (25×25 micro tiles total). The unit of regional coherence. The thing that creates a playable area.
...
- **Micro** is the right level for: collision, traversal, visual rendering, auto-tiling transitions
...
### 3.2 World Unit Tile (L1)

#### 3.2.1 What It Is

A world unit tile is a 5×5 grid of micro tiles that together form a recognizable local structure: a stretch of river, a wall with a gate, a bridge crossing, a fenced enclosure, a clearing, a corridor turn. It is the smallest unit of **designed intent** — while micro tiles are atomic textures and collision primitives, world unit tiles are architectural motifs.
...
Four edge signatures (north, south, east, west), each consisting of a vector of 5 micro-tile edge connector tags (one per micro tile along that border). This is the world unit's "handshake" with its neighbors.
...
Connectivity class is the primary input to the macro solver's chain-continuation logic. When the solver starts a river, it must continue the chain with compatible units until it reaches a terminal.
...
#### 3.2.3 World Unit Library: Archetypes

The world unit library should contain at least the following archetype families for the MVP. Each family has multiple specific variants.
...
**River Family**
- Straight north-south (water column through center, grass flanks)
- Straight east-west (water row through center, grass flanks)
- Bend northeast (water turns from north to east)
- Bend northwest (water turns from north to west)
- Bend southeast (water turns from south to east)
- Bend southwest (water turns from south to west)
...
- Wall segment east-west (stone wall across center row)
- Wall segment north-south (stone wall down center column)
- Wall corner (L-shaped wall)
...
- Fence segment (lighter version of wall, same edge mechanics)
- Fence enclosure (rectangular fence with one opening)
- Fence corner (L-shaped fence)
- Fence terminator (fence post)

**Crossing Family**
- Bridge over river north-south (bridge micro tile replaces center of river column)
- Bridge over river east-west (bridge micro tile replaces center of river row)
...
#### 3.2.4 Transform-Safe Variant Generation
...
A macro tile is a 5×5 grid of world unit tiles, making it 25×25 micro tiles in total. It is the unit of **regional gameplay** — large enough to contain a meaningful sequence of obstacles, resources, and pathways, but small enough to be generated, validated, and cached as a single operation.
...
Each side of the macro tile (north, south, east, west) presents an edge contract that is the composition of the 5 world unit edge signatures along that side. This is what the inter-macro stitching solver uses.
...
#### 3.3.3 Macro Tile Assembly Logic
...
5. **Route validation** — After filling (or during, as an incremental check), the solver verifies that declared route corridors are actually traversable at the micro level. It runs a BFS through the composite 25×25 micro grid from each entry point.
...
| Property | Micro Tile (L0) | World Unit (L1) | Macro Tile (L2) |
|----------|----------------|-----------------|-----------------|
| Grid size | 1×1 cell | 5×5 cells | 25×25 cells (5×5 world units) |
| Source pixel size | 32×32 | 160×160 | 800×800 |
| Isometric projected size | 64×32 | ~320×160 | ~1600×800 |
| Count per parent | 25 per world unit | 25 per macro tile | N/A (top spatial level) |
| Primary concern | Collision, visual texture | Local structure, edge matching | Regional routes, progression |
| Metadata weight | Visual + physics | Structural + connectivity | Strategic + gameplay |

### 4.2 Upward Aggregation

Each tier's metadata is an aggregation of the tier below:

- A world unit tile's edge signature is composed from the edge connectors of the 5 micro tiles along each border
- A world unit tile's traversal mask is composed from the traversal classes of its 25 constituent micro tiles
- A macro tile's edge contract is composed from the edge signatures of the 5 world unit tiles along each border
- A macro tile's route corridors are validated against the composed traversal mask of all 625 constituent micro tiles
...
- The macro solver decides "this position gets a river-chain world unit" → the world unit selector picks from the river library → the micro tiles are determined by the world unit template
- The macro solver decides "this macro tile's east border must be water" → the world unit tiles along the east column must present water on their east edges → the micro tiles in the rightmost column of those world units must be water micro tiles
...
This document defines the complete spatial hierarchy for Emily's Game world engine. The three tiers — micro tile, world unit tile, and macro tile — provide the structural grammar from which procedurally generated worlds are assembled.

The key takeaway is that **metadata is the real product**. The visual SVGs are interchangeable cosmetic expressions. The traversal classes, edge connectors, movement channels, anchor points, and connectivity classes are what make the world engine actually work. Every subsequent document in this series (edge contracts, solver pipeline, rendering, population) operates on these metadata contracts.
