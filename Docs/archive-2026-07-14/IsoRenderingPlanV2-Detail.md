...
2. Call the existing MCP `render_svg_preview` tool with `isometric_preview: true`
3. Receive rendered 256×128 diamond preview + base64 thumb
4. Iterate ("make taller, add height shadow, blend left/right edges")
5. Final output saved as `assets/rock-wall-straight.svg` + `rock-wall-straight.json` (metadata)
...
- Ensure full compatibility with the existing MCP SVG rendering tool
...
Once this experiment successfully demonstrates true height, continuous features, and superior visuals, we will evaluate which components to port back into the main codebase.
