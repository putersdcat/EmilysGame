...
| 14 | ✅ **FULLY RESOLVED (2026-07-13).** Fetched #116's exact body text: its own strict acceptance criteria (braids+spiky hair, eye color affecting the sprite, ≥1 new hat type) are fully met and exceeded (3 new hat/accessory types delivered: cowboy_hat, wizard_hat, flower_crown; outfit patterns match spec exactly: plain/floral/striped/starry). From #116's own broader Phase 2/3 task list (not part of its acceptance criteria), all 5 remaining items now delivered: (1) coin-count + streak-based unlock condition types (`coin_count`/`streak_length` in `UnlockConditionType`, wired to real `state.inventory.countItem('coin')` / `state.streak.consecutiveCorrect`, 2 new cosmetics — `outfit_treasure_hunter` at 50 coins, `hair_streak_flame` at an 8-streak); (2) eye **shape** selector (`round`/`almond`/`wide`, independent of eye color, `EYE_SHAPE_MULT` multiplier applied on top of each expression's base eye geometry — `round` is an exact no-op vs. pre-existing rendering); (3) **backpack** accessory (new independent `backAccessory` slot, full depiction in back view, strap hints in front/side, so it can be worn alongside a head accessory); (4) **scarf** accessory (new independent `neckAccessory` slot, visible in all 3 poses). All 4 new customizer sections (Eye Shape, Backpack, Scarf, plus the earlier unlock types) wired into the customizer UI, randomize, and save serialization. 21 new Playwright tests (`tests/sprites/eye-shape-backpack-scarf.spec.ts`); full `tests/sprites` suite 165/165 passing; visually verified via screenshot. | issues #5, #102, #116 · `src/ui/customizer.ts` · `src/asset-pipeline/sprites.ts` · `src/config/cosmetics.config.ts` · `src/game/cosmetic-unlocks.ts` · `tests/sprites/cosmetics.spec.ts` · `tests/sprites/eye-shape-backpack-scarf.spec.ts` |
...
4. Spot-checks — **DONE**: audio quality vs #130's claim (#11, see §3 row
   ...
   met; ALL 5 residual items now delivered as of 2026-07-13: coin-count/
   streak-length unlock condition types, eye shape selector, backpack
   accessory, scarf accessory — Finding #14 is fully closed out).
