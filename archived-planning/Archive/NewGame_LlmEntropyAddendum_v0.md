...
The LLM acts as the entropy engine, invoked at key points: Game initialization, player movements to new chunks, and optional interactions (e.g., NPC chats). All calls are asynchronous (via Web Workers) to prevent UI lag, with fallbacks to pure JS RNG if inference delays exceed 1-2 seconds.
...
Step 2: Player Input Translation to Pairs
	...
	•	Mechanism: Keyboard inputs (WASD/arrows) map to verb/noun via a dynamic lookup table (JS object/array in src/mechanics.js).
...
Mathematical Hacking of LLM Outputs
	...
	•	Core Techniques (in src/gen.js):
	◦	Hashing: SHA-256 (JS Crypto API) on full text → hex string. Chunk hex:
...
Mapping Entropy to World/Map Rendering
	•	Overview: Hacked numbers feed into chunk generation (32x32 cells), layered with rules for coherence. This creates modular, meshable tiles rendered via Canvas in src/render.js.
...
Implementation Notes
	•	Code Placement: Entropy logic in src/llm.js (prompts) and src/gen.js (hacking/mapping).
