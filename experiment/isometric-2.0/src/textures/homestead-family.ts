/**
 * homestead-family.ts — reusable rural dwelling wall material factory.
 *
 * First-pass family for cottage / hut / homestead wall materials. The family
 * focuses on repeatable 144×144 structural slices for extruded Nano walls:
 *   - timber-frame wall
 *   - plaster / whitewash wall
 *   - rough wood plank wall
 *
 * The side faces are authored as true wall materials, while the top faces use
 * wooden cap-board slices so the homestead family reads as light framed wall
 * construction rather than masonry.
 */
...
function timberPanelField(spec: HomesteadPaletteSpec): string {
  ...
  for (let y = 0; y < SIZE; y += 48) {
    for (let x = 0; x < SIZE; x += 48) {
      ...
      // Main plaster panel — keeps every 48×48 crop readable as a home wall.
      ...
      // One brace per panel, alternating direction so a straight wall reads as framed construction.
...
export function createHomesteadMaterial(spec: HomesteadPaletteSpec): HomesteadMaterial {
  ...
  function sideBody(): string {
    ...
    const plaster = plasterField(spec);
    return plaster;
  }
