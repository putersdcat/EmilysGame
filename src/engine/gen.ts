...
// Re-export the shared grid-walkable counter (B3 / #253 — moved to world/GridUtils.ts).
// The still-in-gen.ts ObstacleSolver phase fns (addExtraObstacles, placeQuizGates)
// call countWalkableNeighbors; it will follow the solver into its own module in slice 7.
...
// `clusterDecorations`, `scatterDecorations`, and the `hasAdjacentInteractable`
// helper were moved to src/engine/world/Populator.ts (B3 / #253). The public
// functions are re-exported at the top of this file.

// `scatterCollectibles`, `layCoinTrails`, and the `findPathBFS` helper
// were moved to src/engine/world/CollectibleScatterer.ts (B3 / #253).
// The public functions are re-exported at the top of this file.
