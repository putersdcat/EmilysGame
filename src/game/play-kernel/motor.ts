/**
 * motor.ts — player locomotion for the play kernel (PR3).
 *
 ...
 *   2. Axis-slide collision against `isFootprintWalkable` (cell SSOT via walk.ts).
 *   3. Constrained embed recovery: legal teleports only (R ladder → BFS → safe
 *      spawn). Never multi-frame free position writes.
 *   4. Prolonged full-block while input held → legal nudges only.
 *
 * Sim dt is capped by loop.ts before runPlayFrame — this module does **not**
 * re-clamp frameMs. Presentation (sprites, camera, footsteps) stays in hooks.
 *
 * @see memories/repo/design-play-kernel-2026-07-19.md (PR3)
 */

import type { GameState } from '../game-state';
import type { ChunkData } from '../../types/game.types';
import { isFootprintWalkable } from './walk';
...
/** Max wall-clock catch-up per frame (tab refocus / hitch). Loop owns clamp. */
 ...
 * Teleports to a legal center; never free-writes through solids/water.
 */
export function resolveEmbedIfNeeded(state: GameState): EmbedResolveResult {
  ...
  // Step 4: stay illegal; visual only; no free-write integrate this frame
  ...
  if (!_embedToastShown) {
    ...
    console.assert(
      false,
      '[play-kernel/motor] embed recovery ladder exhausted; staying put (legal only)',
    );
...
/**
 * Name kept for call-site clarity; constrained recovery only.
...
/**
 * Try legal stuck nudges. Returns true if a nudge committed.
 */
 ...
 * **Every** commit requires isFootprintWalkable.
 */
 ...
 *
 * **No re-clamp of frameMs** — loop.ts already publishes
 * `min(max(raw,0), MOVE_MAX_CATCHUP_MS)`. Callers that inject large dt must
 * go through the loop; direct integrate trusts the published value.
 *
 * Critical: embed step-4 "stuck" must NOT freeze the motor forever — if the
 * player is on illegal ground we still try to walk into legal cells.
 */
export function integrateMovementFrame(
  ...
  // Loop owns catch-up clamp; motor only guards non-finite / negative.
  const remaining0 =
    Number.isFinite(frameMs) && frameMs > 0 ? frameMs : 0;
  if (
    typeof import.meta !== 'undefined' &&
    (import.meta as { env?: { DEV?: boolean } }).env?.DEV &&
    remaining0 > MOVE_MAX_CATCHUP_MS + 1e-6
  ) {
    console.assert(
      false,
      '[play-kernel/motor] simDtMs exceeds MOVE_MAX_CATCHUP_MS — loop should have capped',
    );
  }
  let remaining = remaining0;
  const usedMs = remaining0;
  ...
  if (
    ...
    console.assert(
      false,
      '[play-kernel/motor] post-condition: footprint must be walkable after integrate',
    );
  }

  const displacement = Math.hypot(state.player.x - x0, state.player.y - y0);
  // Report to sim-dt (inject latch + observability)
  noteInjectIntegrateResult(displacement, usedMs);
